...
from pytorch_mbpo.pytorch_sac.agent import Agent
...
class SACAgent(Agent):
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, logger, step):
        ...
        metrics = {"train/agent/critic_loss": critic_loss}
        return metrics
    ...
    def update_actor_and_alpha(self, obs, logger, step):
        ...
        metrics = {
            "train/agent/actor_loss": actor_loss,
            "train/agent/entropy": -log_prob.mean(),
            "train/agent/alpha_loss": alpha_loss,
            "train/agent/alpha": self.alpha
        }
        return metrics
    ...
    def update(self, replay_buffer, logger, step):
        ...
        metrics = {"train/agent/batch_reward": reward.mean()}
        # logger.log('train/batch_reward', reward.mean(), step)

        metrics.update(self.update_critic(obs, action, reward, next_obs, not_done_no_max,
                           logger, step))

        if step % self.actor_update_frequency == 0:
            metrics.update(self.update_actor_and_alpha(obs, logger, step))
